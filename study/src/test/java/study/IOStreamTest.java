...
class IOStreamTest {
    ...
    class FilterStream_학습_테스트 {
        ...
        /**
         * BufferedInputStream은 데이터 처리 속도를 높이기 위해 데이터를 버퍼에 저장한다. -> 하드디스크에서 1byte씩 읽어오는 것이 아닌, 파일 내용을 2^13 byte 단위로 메모리(버퍼)에 올려놓고 버퍼에서 읽어서 처리
         * InputStream 객체를 생성하고 필터 생성자에 전달하면 필터에 연결된다.
         * 버퍼 크기를 지정하지 않으면 버퍼의 기본 사이즈는 얼마일까? -> 2^13
         */
        @Test
