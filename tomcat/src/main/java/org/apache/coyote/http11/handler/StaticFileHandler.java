    ...
    public static HttpResponse handle(HttpRequest httpRequest, URL resourceUrl) {
        try {
            ...
            if (httpRequest.getPath().endsWith(".css")) {
                return new HttpResponse(HttpStatus.OK, "text/css;charset=utf-8", responseBody);
            }
            return new HttpResponse(HttpStatus.OK, "text/html;charset=utf-8", responseBody);
        } catch (IOException | URISyntaxException exception) {
            logger.error(exception.getMessage(), exception);
            return new HttpResponse(HttpStatus.NOT_FOUND, "text/html;charset=utf-8", null);
        }
    }
