...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void OutputStream은_데이터를_바이트로_처리한다() throws IOException {
            ...
            assertThat(outputStream).hasToString("nextstep");
            outputStream.close();
        }
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
            ...
            //given
            byte[] bytes = new byte[]{-16, -97, -92, -87};
            String actual;

            //when
            try (final InputStream inputStream = new ByteArrayInputStream(bytes)) {
                final byte[] readBytes = inputStream.readNBytes(bytes.length);

                actual = new String(readBytes);
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() throws IOException {
            ...
            try (
                ...
                bufferedReader.lines()
                    .forEach(line -> stringBuilder.append(line).append(System.lineSeparator()));
            }
            //then
