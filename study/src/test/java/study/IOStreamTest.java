...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
             ...
             */
            outputStream.flush();
            //ByteArrayOutputStream은 메모리에 바로 쓰기 때문에 flush()가 아무 일도 하지 않는다.
            // 반면 BufferedOutputStream은 내부 버퍼(기본 8192바이트)에 모아두었다가 flush()를 호출해야 실제로 내보냄.
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
            ...
            //이 byte[]를 UTF-8이라는 문자 인코딩 규칙으로 해석해서 문자열로 만들어라.
    ...
    class FilterStream_학습_테스트 {
         ...
         * 버퍼 크기를 지정하지 않으면 버퍼의 기본 사이즈는 얼마일까? 8KiB
         * 여러 번의 작은 read 대신 데이터를 일정량 미리 읽어 메모리에 보관한다
         */
        @Test
