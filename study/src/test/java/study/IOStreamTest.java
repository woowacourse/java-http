...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void OutputStream은_사용하고_나서_close_처리를_해준다() throws IOException {
             ...
             */
            try (outputStream) {
                /** 변수가 try-with-resources에 들어가려면 두 조건이 필요하다.
                 *  1. 객체의 타입이 AutoCloseable 인터페이스를 구현해야 한다.
                 *  2. 미리 선언한 변수를 넣는다면 final 또는 effectively final이어야 한다.
                 *      - 변수지만 새로 대입이 되지 않았다면 effectively final이다.
                 */
                // 블록을 나가는 순간 close() 자동 호출
            }
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_사용하고_나서_close_처리를_해준다() throws IOException {
             ...
             */
            try (inputStream) {}
            // ByteArrayInputStream은 메모리 참조이기 때문에 GC가 회수할 수 있지만,
            // FileInputStream 같은 외부 자원을 사용하는 스트림은 close 하지 않으면 자원 누수가 발생한다.
