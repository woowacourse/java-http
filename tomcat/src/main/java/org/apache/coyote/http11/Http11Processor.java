...
public class Http11Processor implements Runnable, Processor {
    ...
    private Response getResponse(final RequestLine requestLine, final RequestHeader requestHeader, RequestBody requestBody) {
    ...
    private Response login(final RequestHeader requestHeader, final RequestBody requestBody) {
        try {
            Session savedSession = findSession(requestHeader);
    ...
    private Session findSession(final RequestHeader requestHeader) {
        String[] cookies = requestHeader.getHeaderValue("Cookie").split("; ");
        for(String cookie : cookies) {
            if (cookie.contains("JSESSIONID")) {
                Cookie httpCookie = Cookie.from(cookie);
    ...
    private Response register(final RequestBody requestBody) {
        if (requestBody == null) {
            return Response.redirection("401.html");
        }
        String account = requestBody.getContentValue("account");
     ...
     */
    private Response responseStaticFile(String requestUri, RequestHeader requestHeader) {
        try {
