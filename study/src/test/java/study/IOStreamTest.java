...
//            final ByteArrayOutputStream outputStream = mock(ByteArrayOutputStream.class);
             ...
             * ByteArrayOutputStream과 어떤 차이가 있을까? TODO: 뭔차이?
             */
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
             ...
             */
            byte[] bytesRs = new byte[inputStream.available()];
            for (int i = 0; i < bytesRs.length; i++) {
                bytesRs[i] = (byte) inputStream.read();
            }
            final String actual = new String(bytesRs);
    ...
    class FilterStream_학습_테스트 {
        ...
        /**
         * BufferedInputStream은 데이터 처리 속도를 높이기 위해 데이터를 버퍼에 저장한다. TODO: 아니면 어디에 저장하지? InputStream 객체를 생성하고 필터 생성자에 전달하면
