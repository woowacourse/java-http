...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
             ...
             * - 목적: ByteArrayOutputStream은 메모리 저장, BufferedOutputStream은 성능 최적화
             * - 사용법: ByteArrayOutputStream은 독립적 사용, BufferedOutputStream은 다른 스트림을 래핑
             * - 데이터 접근: ByteArrayOutputStream은 저장된 데이터에 직접 접근 가능, BufferedOutputStream은 중간 버퍼 역할만
             */
            outputStream.flush();
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
             ...
             * - 바이트 값을 문자열로 변경하려면 Reader/Writer를 사용, InputStreamReader
             * - inputStream을 모두 읽어 EOF에 도달하면 -1
             */
    ...
    class FilterStream_학습_테스트 {
         ...
         * - 기본 사이즈: 8192 -> 8KB -> 왜 8KB?
         * - 많은 파일시스템의 기본 블록 크기가 4KB-8KB 범위. 8KB 버퍼는 파일시스템의 블록과 잘 정렬되어 I/O 성능을 최적화
         */
        @Test
    ...
    class InputStreamReader_학습_테스트 {
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() throws IOException {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                // 왜 System.lineSeparator()를 사용하면 실패할까?
                // - 원본 문자열은 \r\n 사용, Unix/Linux/macOS: "\n" 사용 -> macOS 에서 실행하면 실패
            }
