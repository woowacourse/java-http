...
/**
 * 입출력(I/O)은 하나의 시스템에서 다른 시스템으로 데이터를 이동 시킬 때 사용한다. 자바는 스트림(Stream)으로부터 I/O를 사용한다.
 * <p>
...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
             ...
             * -> ByteArrayOutputStream은 바이트 배열이므로 끝을 안다. 닫아주지 않아도 된다. 다 작성하면 그때 전송한다.
             * -> BufferedOutputStream은 버퍼링 크기만큼 heap메모리에 작성하다가 버퍼가 가득차면 한번에 write한다.
             * -> 즉, 가득차지 않으면 전송하지 않는다.
             * -> 따라서 BufferedOutputStream은 버퍼가 가득 차지 않았지만 전송하고자 하는 내용이 다 됐으면 flush, close 하여 전송해야한다.
             */
            outputStream.flush();
    ...
    class FilterStream_학습_테스트 {
         ...
         * <p></p>
         * -> BufferedInputStream default buffer size : 2048byte
         * <p></p>
         * -> BufferedOutputStream default buffer size : 512byte
         */
        @Test
        void 필터인_BufferedInputStream를_사용해보자() throws IOException {
            ...
            final InputStream inputStream = new ByteArrayInputStream(bytes);
            final InputStream bufferedInputStream = new BufferedInputStream(inputStream, bytes.length);
     ...
     * 문자로 변환할 수 있다. reader, writer를 사용하면 입출력 스트림을 바이트가 아닌 문자 단위로 데이터를 처리하게 된다.
     */
    @Nested
    class InputStreamReader_학습_테스트 {
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() throws IOException {
            ...
            final String emoji = String.join(delimiter,
                    ...
                    "😋😛😝😜🤪🤨🧐🤓😎🥸🤩",
                    "");
            final byte[] bytes = emoji.getBytes();
            final InputStream inputStream = new ByteArrayInputStream(bytes);
            ...
            while ((line = bufferedReader.readLine()) != null) {
                actual.append(String.format("%s%s", line, delimiter));
            }
