...
- 문제 상황
    - acceptCount=2, maxThreads=3으로 세팅하고 Jmeter를 통해 10개의 동시 요청 테스트를 진행함
    - 3개의 요청은 거부 메세지가 클라이언트에게 전달됨
    - 나머지 2개의 요청은 클라이언트에게 예외 스택 트레이스가 전달됨
- 원인
    - 현재 TCP 연결수 = (스레드풀의 스레드 수 + 스레드풀의 대기열수)
    - 이로인해 초과 요청이 들어오면 TCP 연결 자체가 되지 않게되고, 이로인해 일부 초과 요청에 대해 거부메세지가 전달되지 않음
- 해결방법
    - TCP 연결수 = (스레드풀의 스레드 수 + 스레드풀의 대기열수) * 2 로 설정
