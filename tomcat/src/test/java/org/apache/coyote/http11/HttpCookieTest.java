...
class HttpCookieTest {
    ...
    void get_session_true_o() {
        HttpCookie cookie = HttpCookie.from("JSESSIONID=" + sessionId);
        Session session = cookie.getSession(true);
    ...
    void get_session_false_o() {
        HttpCookie cookie = HttpCookie.from("JSESSIONID=" + sessionId);
        Session session = cookie.getSession(false);
    ...
    void get_session_true_throw() {
        HttpCookie cookie = HttpCookie.from("JSESSIONID=" + UUID.randomUUID());

        assertThatThrownBy(() ->  cookie.getSession(true))
                .isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    @DisplayName("세션 관리자에 저장되지 않은 JSESSIONID로 세션을 찾으면 예외가 발생한다. - false 전달")
    void get_session_false_throw() {
        HttpCookie cookie = HttpCookie.from("JSESSIONID=" + UUID.randomUUID());

        assertThatThrownBy(() ->  cookie.getSession(false))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
