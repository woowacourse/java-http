...
class ConcurrencyTest {
    ...
    void test() throws InterruptedException {
        ...
        // 디버그 중단점까지 두 개의 스레드가 도달하면, 유저가 포함되어 있는지에 대한 조건문을 통과한 상태이다.
        // 따라서 각각의 스레드가 유저를 저장해버려서 크기가 2가 된다.
        // 이를 해결하기 위해서는 join 메서드에 synchronized 설정을 해준다.
        // 하나의 스레드가 join 메서드를 사용하는 동안, 다른 스레드가 join 메서드를 사용하지 못한다.
        // 따라서, 하나의 스레드가 유저를 저장하고, 다른 스레드는 유저 포함 조건에 걸려서 유저를 저장하지 않는다.
        // 따라서 크기가 1로 된다.
