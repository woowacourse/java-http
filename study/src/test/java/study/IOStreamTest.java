...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
             ...
             * => ByteArrayOutputStream은 바로바로 메모리 쓰기 작업을 해야하는데,
             * BufferedOutputStream의 경우에는 데이터들을 중간 버퍼에 모으고 버퍼가 차거나 flush()/close()가 호출되면
             * 한번에 전달하는 형태라서 메모리 쓰기 작업을 최소화할 수 있다.
             */
            outputStream.flush();
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
             ...
             */
            final String actual = new String(inputStream.readAllBytes(), "UTF-8");
