...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            /**
             * BufferedOutputStream은 버퍼링을 지원하는 출력 스트림이다.
             * flush()를 호출하면 버퍼에 남은 데이터를 감싸고 있는 출력 스트림으로 전달한다.
             * 현재 확인한 JDK 구현에서는 버퍼 크기를 별도로 지정하지 않으면
             * 일반 스레드에서 8192바이트의 버퍼를 사용한다.
             */
             ...
             * BufferedOutputStream은 생성할 때 다른 출력 스트림을 받는다.
             * ByteArrayOutputStream은 다른 출력 스트림 없이도 생성할 수 있다.
             */
            outputStream.flush();
    ...
    class FilterStream_학습_테스트 {
         ...
         * 않으면 버퍼의 기본 사이즈는 얼마일까? DEFAULT_BUFFER_SIZE = 8192 (8KiB)
         */
        @Test
