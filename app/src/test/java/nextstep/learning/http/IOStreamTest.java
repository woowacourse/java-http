 ...
 * 바이트가 아닌 <b>텍스트(문자)</b>를 읽고 쓰려면 <b>Reader와 Writer</b> 클래스를 연결한다.<br> Reader, Writer는 다양한 문자 인코딩(e.g. UTF-8)을 처리할 수
 * 있다.
 */
...
class IOStreamTest {
    ...
    /**
     * 자바의 기본 출력 클래스는 <b>java.io.OutputStream</b>이다.<br> OutputStream의 <b>write(int b)</b> 메서드는 기반 메서드이다.<br><br>
    ...
    class OutputStream_학습_테스트 {
        ...
        /**
         * OutputStream은 다른 매체에 바이트로 데이터를 쓸 때 사용한다.<br> OutputStream의 서브 클래스(subclass)는 특정 매체에 데이터를 쓰기 위해 <b>write(int
         * b)</b> 메서드를 사용한다.<br> 예를 들어, FilterOutputStream은 파일로 데이터를 쓸 때,<br> DataOutputStream은 자바의 primitive type data를
         * 다른 매체로 데이터를 쓸 때 사용한다.<br>
         * <br>
        ...
        void OutputStream은_데이터를_바이트로_처리한다() throws IOException {
            ...
            // OutputStream의 toString()이 아닌, ByteArrayOutputStream의 toString() 메소드가 실행됨
    ...
    /**
     * 자바의 기본 입력 클래스는 <b>java.io.InputStream</b>이다.<br> InputStream은 다른 매체로부터 바이트로 데이터를 읽을 때 사용한다.<br> InputStream의
     * <b>read()</b> 메서드는 기반 메서드이다.<br><br>
    ...
    class InputStream_학습_테스트 {
        ...
        /**
         * read() 메서드는 매체로부터 단일 바이트를 읽는데, 0부터 255 사이의 값을 int 타입으로 반환한다.<br> int 값을 byte 타입으로 변환하면 -128부터 127 사이의 값으로
         * 변환된다.<br> 그리고 Stream 끝에 도달하면 <b>-1</b>을 반환한다.<br>
         */
        @Test
    ...
    /**
     * 필터는 필터 스트림, reader, writer로 나뉜다.<br> 필터는 바이트를 다른 데이터 형식으로 변환 할 때 사용한다.<br> reader, writer는 UTF-8, ISO 8859-1 같은
     * 형식으로 인코딩된 텍스트를 처리하는 데 사용된다.
     */
    @Nested
    ...
    /**
     * 자바의 기본 문자열은 UTF-16 유니코드 인코딩을 사용한다. 바이트를 문자(char)로 처리하려면 인코딩을 신경 써야 한다. InputStreamReader를 사용하면 지정된 인코딩에 따라 유니코드
     * 문자로 변환할 수 있다. reader, writer를 사용하면 입출력 스트림을 <b>바이트</b>가 아닌 <b>문자</b> 단위로 데이터를 처리하게 된다.
     */
    @Nested
