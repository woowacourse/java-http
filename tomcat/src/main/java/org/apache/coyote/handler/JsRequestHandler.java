...
import static org.apache.coyote.common.MediaType.APPLICATION_JAVASCRIPT;

public class JsRequestHandler implements RequestHandler {
    ...
    private ResponseBody parseToResponseBody(final HttpRequest httpRequest) {
        try {
            ...
            throw new CoyoteHttpException("응답할 js 파일을 읽던 도중에 오류가 발생하였습니다.");
        }
    }

    private Headers parseToResponseHeader(final ResponseBody responseBody) {
        final Map<String, String> responseHeaders = new HashMap<>();
        responseHeaders.put(CONTENT_TYPE.source(), APPLICATION_JAVASCRIPT.source());
