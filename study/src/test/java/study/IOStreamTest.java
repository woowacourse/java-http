...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            ...
            System.out.println("BufferedOutputStream 시간" + (l2 - l));

            //
            final long l3 = System.currentTimeMillis();
            final OutputStream outputStream2 = mock(ByteArrayOutputStream.class);

            outputStream2.flush();

            verify(outputStream2, atLeastOnce()).flush();
            outputStream2.close();
            final long l4 = System.currentTimeMillis();
            System.out.println("ByteArrayOutputStream 시간" + (l4 - l3));
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
            ...
            try (inputStream){
                actual = new String(inputStream.readAllBytes(), Charset.defaultCharset());
            }
...
//        }
        ...
        void 필터인_BufferedInputStream를_사용해보자() throws IOException {
            ...
            final long l3 = System.currentTimeMillis();
            final String text2 = "필터에 연결해보자.";
            final InputStream inputStream2 = new ByteArrayInputStream(text2.getBytes());
            final InputStream bufferedInputStream2 = new BufferedInputStream(inputStream2);

            final byte[] actual2 = new byte[text.getBytes().length];
            bufferedInputStream2.read(actual2);

            assertThat(actual2).isEqualTo("필터에 연결해보자.".getBytes());
            final long l4 = System.currentTimeMillis();
            assertThat(bufferedInputStream2).isInstanceOf(FilterInputStream.class);
