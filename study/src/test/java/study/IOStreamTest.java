...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
             ...
             * -> ByteArrayOutputStream 는 write() 메서드를 수행할 때 마다 출력 스트림을 소모한다.
             * 그에 반해 BufferedOutputStream은 데이터를 버퍼에 임시 저장 후 버퍼가 가득 차면 한번 에 출력 스트림을 사용한다.
             * 그러나 BufferedOutputStream 은 버퍼가 가득 차지 않은 경우 Stream은 동기로 동작하기 때문에 버퍼가 찰 때까지 기다라다 교착 상태가 된다.
             */
            //
            outputStream.flush();
    ...
    class FilterStream_학습_테스트 {
         ...
         * 않으면 버퍼의 기본 사이즈는 얼마일까? -> 버퍼의 기본 사이즈는 8192 bytes
         */
        @Test
