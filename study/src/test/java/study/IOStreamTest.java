...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void OutputStream은_데이터를_바이트로_처리한다() throws IOException {
            final byte[] bytes = {110, 101, 120, 116, 115, 116, 101, 112};  // n e x t s t e p

            // 인수 없이도 생성 가능. 기본 사이즈는 32이며, 자동 확장 가능
            ...
            outputStream.close();  // ByteArrayOutputStream은 메모리 기반 스트림이므로 close() 메서드를 호출해도 아무런 효과가 없지만, 일관성을 위해 닫아줌
        }
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            ...
            bufferedOutputStream.flush();  // 기본 버퍼 사이즈는 8192 바이트. 버퍼가 채워지기 전 명시적 flush().
            final String actual = outputStream.toString();
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
            ...
            // readAllBytes() 메서드는 InputStream의 모든 바이트를 읽어서 바이트 배열로 반환한다.
            final String actual = new String(inputStream.readAllBytes());

            assertThat(actual).isEqualTo("🤩");
            assertThat(inputStream.read()).isEqualTo(-1);  // 스트림 끝에 도달하면 -1 반환
            inputStream.close();
        }
    ...
    class FilterStream_학습_테스트 {
        ...
        void 필터인_BufferedInputStream를_사용해보자() throws IOException {
            ...
            // BufferedInputStream은 기본 버퍼 사이즈가 8192 바이트이다.
    ...
    class InputStreamReader_학습_테스트 {
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() {
            ...
            bufferedReader.lines().forEach(str -> actual.append(str).append(System.lineSeparator()));
