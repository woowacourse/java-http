...
class ConcurrencyTest {
    ...
    void test() throws InterruptedException {
        ...
        // 답변: 디버거를 통해 스레드를 일시 중지하면, 하나의 스레드가 join() 메서드의 조건을 평가하고 리스트에 gugu가 없다고 판단한 후,
        // 그 스레드가 다음 단계로 넘어가기 전에 다른 스레드도 같은 조건을 평가할 수 있는 시간을 제공합니다.
        // 이로 인해 두 스레드가 모두 리스트에 gugu가 없다고 판단하고, 결국 두 개의 gugu가 리스트에 추가될 수 있습니다.
