...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            ...
            /**
             * ByteArrayOutputStream : 바이트배열에 데이터를 쓰기 위한 출력 스트림
             * 객체가 생성될 때, 쓰여진 데이터를 저장할 바이트 배열이 생성되며 데이터를 기록할 때마다 해당 바이트 배열에 데이터가 쌓인다.
             *
             *
             * BufferedOutputStream : 출력 스트림에 버퍼(Buffer) 기능을 더해 주는 바이트 기반의 보조 스트림
             * 데이터를 하나씩 바로 쓰지 않고 메모리 버퍼에 모아두었다가 가득 차면 한 번에 전송하므로 입출력 성능이 좋아진다.
             */
            outputStream.flush();
