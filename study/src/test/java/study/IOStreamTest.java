...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
         ...
         *
         * write(int b) 메서드는 데이터를 바이트로 출력하기 때문에 비효율적이다.
        ...
        void OutputStream은_데이터를_바이트로_처리한다() throws IOException {
             ...
             */
            outputStream.write(bytes); // 노트 같은 공간으로 일단 이해!
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            ...
            // BufferedOutputStream: buffer 관리 & FilterOutputStream을 상속받았기 때문에 생성 시 OutputStream 전달
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
             ...
             */
            byte[] resultBytes = inputStream.readAllBytes(); // 연필 같은 역할로 일단 이해!
            final String actual = new String(resultBytes);

            assertThat(actual).isEqualTo("🤩");
            assertThat(inputStream.read()).isEqualTo(-1); // read => inputStream 소모
            inputStream.close();
        }
    ...
    class FilterStream_학습_테스트 {
         ...
         * 버퍼 크기를 지정하지 않으면 버퍼의 기본 사이즈는 얼마일까? ======> 8192 byte 입니다!
         */
        @Test
        void 필터인_BufferedInputStream를_사용해보자() throws IOException {
            ...
            // todo
            final InputStream bufferedInputStream = new BufferedInputStream(inputStream); // extends FilterInputStream
            // BufferedInputStream 생성자 - public
            // FilterInputStream 생성자 - protected

            final byte[] actual = bufferedInputStream.readAllBytes();

            assertThat(bufferedInputStream).isInstanceOf(FilterInputStream.class);
            assertThat(actual).isEqualTo("필터에 연결해보자.".getBytes());
            assertThat(bufferedInputStream.read()).isEqualTo(-1); // 마찬가지로 read => inputStream 소모
        }
    }
    ...
    class InputStreamReader_학습_테스트 {
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() throws IOException {
            ...
            // BufferedReader: 스트림에서 소량의 데이터만 읽을 수 있기 때문에 대규모 데이터 처리에 적합하다.
