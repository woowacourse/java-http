...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        @Test
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다_test() throws IOException {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            ...
            OutputStream outWithNoBuf = mock(OutputStream.class);
            OutputStream outputStream = new BufferedOutputStream(out);

            for (int i = 0; i < 10; i++) {
                outputStream.write(bytes);
                outWithNoBuf.write(bytes);
            }
            outputStream.flush();
            verify(out, times(1))
                    .write(any(), anyInt(), anyInt());
            verify(outWithNoBuf, times(10))
                    .write(any());
        }
