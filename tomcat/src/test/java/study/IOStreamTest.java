...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
         ...
         * 동기(synchronous)로 동작하기 때문에 버퍼가 찰 때까지 기다리면 데드락(deadlock) 상태가 되기 때문에 flush로 해제한다.
         */
        @Test
    ...
    class InputStream_학습_테스트 {
        ...
        void InputStream은_데이터를_바이트로_읽는다() throws IOException {
            ...
            for (int i = 0; i < length; i++) {
                inputBytes[i] = (byte) inputStream.read();
            }

            final var actual = new String(inputBytes);
        ...
        /**
         * 필터는 필터 스트림, reader, writer로 나뉜다. 필터는 바이트를 다른 데이터 형식으로 변환 할 때 사용한다. reader, writer는 UTF-8, ISO 8859-1 같은 형식으로
        ...
        class FilterStream_학습_테스트 {
            ...
            /**
             * BufferedInputStream은 데이터 처리 속도를 높이기 위해 데이터를 버퍼에 저장한다. InputStream 객체를 생성하고 필터 생성자에 전달하면 필터에 연결된다. 버퍼 크기를
             * 지정하지 않으면 버퍼의 기본 사이즈는 얼마일까?
             */
            @Test
