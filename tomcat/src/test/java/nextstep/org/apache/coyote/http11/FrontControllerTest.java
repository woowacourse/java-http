...
class FrontControllerTest {
    ...
    void handleRoot() throws IOException {
        ...
        assertThat(response.getContentType()).isEqualTo(ContentType.withCharset(HTML.toString()));
    ...
    void handleHTML(final String URI) throws IOException {
        final Response response = FrontController.handle(
                Request.of("get", URI, Map.of(ACCEPT, "text/html"), "")
        );
    ...
    void handleCSS() throws IOException {
        final Response response = FrontController.handle(
                Request.of("get", "/css/styles.css", Map.of(ACCEPT, "text/html"), "")
        );

        assertThat(response.getStatus()).isEqualTo(Status.OK);
        assertThat(response.getContentType()).isEqualTo(ContentType.withCharset(CSS.toString()));
    }
    ...
    void handleNotFound() throws IOException {
        final Response response = FrontController.handle(
                Request.of(
                        "get", "/neverexist/not.css", Map.of(ACCEPT, "text/html"), "")
        );
