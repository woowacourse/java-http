...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        /**
         * OutputStream은 다른 매체에 바이트로 데이터를 쓸 때 사용한다.<br> OutputStream의 서브 클래스(subclass)는 특정 매체에 데이터를 쓰기 위해 <b>write(int b)</b>
         * 메서드를 사용한다.<br> 예를 들어, FilterOutputStream은 파일로 데이터를 쓸 때,<br> DataOutputStream은 자바의 primitive type data를 다른 매체로 데이터를
         * 쓸 때 사용한다.<br>
         * <br>
    ...
    /**
     * 필터는 필터 스트림, reader, writer로 나뉜다.<br> 필터는 바이트를 다른 데이터 형식으로 변환 할 때 사용한다.<br> reader, writer는 UTF-8, ISO 8859-1 같은 형식으로
     * 인코딩된 텍스트를 처리하는 데 사용된다.
     */
    @Nested
    ...
    /**
     * 자바의 기본 문자열은 UTF-16 유니코드 인코딩을 사용한다. 바이트를 문자(char)로 처리하려면 인코딩을 신경 써야 한다. InputStreamReader를 사용하면 지정된 인코딩에 따라 유니코드 문자로
     * 변환할 수 있다. reader, writer를 사용하면 입출력 스트림을 <b>바이트</b>가 아닌 <b>문자</b> 단위로 데이터를 처리하게 된다.
     */
    @Nested
