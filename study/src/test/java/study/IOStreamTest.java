    ...
    class InputStreamReader_학습_테스트 {
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() throws IOException {
            final String emoji = String.join("\r\n",
                    ...
                    "😋😛😝😜🤪🤨🧐🤓😎🥸🤩",
                    "");
            final InputStream inputStream = new ByteArrayInputStream(emoji.getBytes()); // byte 단위로 읽어오는 stream
            final InputStreamReader inputStreamReader = new InputStreamReader(inputStream); // 버퍼 stream을 기반으로 reader 생성
            final BufferedReader bufferedReader = new BufferedReader(inputStreamReader);

            final String collectedLines = bufferedReader.lines()
                .collect(Collectors.joining("\r\n"));

            final StringBuilder actual = new StringBuilder(collectedLines);
            actual.append("\r\n");
