...
class IOStreamTest {
    ...
    class FilterStream_학습_테스트 {
         ...
         * 버퍼 크기를 지정하지 않으면 버퍼의 기본 사이즈는 얼마일까? 8192 Bytes
         */
        @Test
        void 필터인_BufferedInputStream를_사용해보자() throws IOException {
            final String text = "필터에 연결해보자.";
            final InputStream inputStream = new ByteArrayInputStream(text.getBytes());
            final InputStream bufferedInputStream = new BufferedInputStream(inputStream, text.length());
    ...
    class InputStreamReader_학습_테스트 {
        ...
        void BufferedReader를_사용하여_문자열을_읽어온다() throws IOException {
            ...
            bufferedReader.lines().map(line->line+"\r\n").forEach(actual::append);
