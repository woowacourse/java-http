...
public class Http11Processor implements Runnable, Processor {
    ...
    private HttpResponse respond(final HttpRequest httpRequest) {
        ...
        } catch (InvalidHttpMethodException | MethodNotAllowedException e) {
            return toFoundResponse(httpRequest, METHOD_NOT_ALLOWED_PATH);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    private HttpResponse toOkResponse(final HttpRequest httpRequest, final String responseBody) {
        ...
        responseHeaders.add("Content-Type", contentType.getValue() + ";charset=utf-8");
    ...
    private void handleSession(final HttpRequest httpRequest, final HttpHeaders responseHeaders) {
        HttpCookie httpCookie = httpRequest.getCookie();
        if (!httpCookie.has(JSESSIONID_KEY)) {
            String sessionId = UUID.randomUUID().toString();
            createSession(sessionId);
            responseHeaders.add("Set-Cookie", JSESSIONID_KEY + "=" + sessionId);
        }
    }
