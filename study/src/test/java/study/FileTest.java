...
class FileTest {
    ...
    void resource_디렉터리에_있는_파일의_경로를_찾는다() throws URISyntaxException {
        ...
        // todo
        var resourceUri = Objects.requireNonNull(getClass().getClassLoader().getResource(fileName)).toURI();

        Path actualPath = Paths.get(resourceUri);

        System.out.println("생성된 경로: " + actualPath);

        assertThat(actualPath.toString()).endsWith(File.separator + fileName);
    }
