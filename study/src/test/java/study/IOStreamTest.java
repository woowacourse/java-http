...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        void BufferedOutputStream을_사용하면_버퍼링이_가능하다() throws IOException {
            ...
            /**
             * 내부 구현을 보면 BufferedOutputStream은 내부에 FilterOutputStream을 가지고 있음.
             * 해당 필터아웃풋 스트림이 실질적으로 데이터가 써지는 스트림이고, 버퍼드아웃풋 스트림에는 해당 데이터가 들어가기 전까지 버퍼링으로 기록을 해둠
             * flush를 호출하거나, capacity에 도달해야 버퍼에 있는 데이터들이 실제 out stream에 작성됨
             *
             * bufferedOutputStream은 플러시전에 내용을 출력하면 버퍼의 내용들은 출력되지 않을것으로 예상됨..
             */
        ...
        void OutputStream은_사용하고_나서_close_처리를_해준다() throws IOException {
            ...
            /**
             * OutputStream은 try-with-resources를 이용하면 자동으로 try가 끝나고 close호출
             *
             * 가능한이유 : OutputStream이 Closable을 implement하여 구현하고 있어서.
             * 해당 Closable은 AutoClosable을 extends
             */
