...
class FileTest {
     ...
     * resource 디렉터리 경로 찾기
     *
     * <p>File 객체를 생성하려면 파일의 경로를 알아야 한다. 자바 애플리케이션은 resource 디렉터리에 HTML, CSS 같은 정적 파일을 저장한다. resource
    ...
    void resource_디렉터리에_있는_파일의_경로를_찾는다() throws URISyntaxException {
        ...
        String actual = Paths.get(resource.toURI()).toString();
     ...
     * 파일 내용 읽기
     *
     * <p>읽어온 파일의 내용을 I/O Stream을 사용해서 사용자에게 전달 해야 한다. File, Files 클래스를 사용하여 파일의 내용을 읽어보자.
     */
    @Test
