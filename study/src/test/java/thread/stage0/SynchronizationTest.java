...
/**
 * 다중 스레드 환경에서 두 개 이상의 스레드가 변경 가능한(mutable) 공유 데이터를 동시에 업데이트하면 경쟁 조건(race condition)이 발생한다. 자바는 공유
 * 데이터에 대한 스레드 접근을 동기화(synchronization)하여 경쟁 조건을 방지한다. 동기화된 블록은 하나의 스레드만 접근하여 실행할 수 있다.
 * <p>
...
class SynchronizationTest {
  ...
  void testSynchronized() throws InterruptedException {
    ...
    IntStream.range(0, 1000)
        .forEach(count -> executorService.submit(() -> synchronizedMethods.syncCal(count)));
    //.forEach(count -> executorService.submit(synchronizedMethods::entireSyncCal));
  ...
  private static final class SynchronizedMethods {
    ...
    public synchronized void entireSyncCal() {
      setSum(getSum() + 1);
    }

    public void syncCal(final int count) {
