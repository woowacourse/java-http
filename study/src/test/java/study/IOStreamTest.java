...
class IOStreamTest {
    ...
    class OutputStream_학습_테스트 {
        ...
        /**
         * BufferedOutputStream은 OutputStream을 감싼 데코레이터로, 쓰기 요청을 내부 버퍼에 모았다가
         * 버퍼가 가득 차거나 flush()/close()가 호출될 때 감싼 스트림에 한 번에 전달한다.
         * 감싼 스트림이 파일이나 소켓처럼 쓰기마다 시스템 콜이 발생하는 경우,
         * 작은 쓰기를 모아 호출 횟수를 줄일 수 있어 효율적이다.
         * 버퍼가 덜 찬 데이터는 flush()를 호출해야 목적지로 전달된다.
         */
        @Test
